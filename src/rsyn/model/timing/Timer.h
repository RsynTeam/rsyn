...
private:
	...
	//! @brief Returns the wire delay from the driver to this sink. If the pin
	...
	//! @brief Returns the worst slack at this timing pin. If the timing is not
	//!        asserted (uninitialized) in this pin, returns the default value
	//!        passed as argument.
	...
	//! @brief Returns the worst slack and the respective transition (rise/fall)
	//!        at a timing pin.
...
public:	
	...
	//!        to it are marked as dirty, so no need to notify the timer again.
	...
	//!        timing, but the timing of driving nets are computed before the
	...
	//! @note  The drivers of a cell are the driver pins of the nets driving the
	...
	//!        requested, this mean that the rise slew at the input pin will be
	//!        returned.
	...
	//! @note  This is not the same as the output slew of the arc as the slew at
	//!        the output pin may have been defined by other most critical arc
	...
	//! @brief Finds the sink with most similar slack to the "to" pin of the arc 
	...
	//!
	//! For instance, for late mode, if the delay of worst path passing through
	//! a pin is 10 and the clock period is 5, the relativity is 2. Similarly,
	...
	//! @brief Returns the cell relativity i.e. the worst output pin relativity
	//!        for combinational cells or the worst relativity among the data 
	...
	//! @brief Returns the net relativity i.e. the worst driver pin relativity.
	...
	//! @note  Centrality is rough measure of how many critical endpoints are
	...
	//! @brief Returns the critical arc driving a pin (to) for a specific output
	//!        transition.
	...
	Rsyn::Arc getCriticalTimingArcToPin(Rsyn::Pin pin, const TimingMode mode) const {
		Rsyn::Arc worstArcPtr = nullptr;
		Number worstSlack = +std::numeric_limits<Number>::max();
	...
	//!        otherwise.
	//! @note  The uninitialized values is a special value used by the timer to
	//!        indicate that the no timing computation was ever propagated or
...
public:
	...
	//!        negative first), that is, the most critical path is stored in the
	//!        index zero.
	//! @note  The first hop in the path is the path startpoint and the last one
	//1        is the endpoint.
	...
	//! @brief Returns one critical path per endpoint. The path from the most
	...
	//!        endpoints. The bi-dimensional path vector returned is sorted by
	//!        decreasing endpoint criticality and then by decreasing path
	//!        criticality. That is, the top most critical path is at index
	//!        [0][0] and the top 3 critical path of top 5 endpoint is at index
	...
	//! @brief Same as queryTopCriticalPathsFromTopCriticalEndpoints(), but the
	...
	//!        generated. If the hop is an output pin, the returned delay is the
